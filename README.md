...
目前已完成核心功能的開發，具備穩定的基礎架構與可靠的行為分析能力。系統架構已為未來的功能擴展 (如錨點系統、多攝影機管理) 預留了清晰的介面。歡迎任何形式的建議與討論。

## 核心特性

-   **高效能推論與編碼管線 (High-Performance Inference & Encoding Pipeline)**:
    -   **AI 推論**: 整合 **YOLO** 物件偵測模型與 **NVIDIA TensorRT** 引擎進行加速，實現高速物件偵測與特徵提取。
    -   **影片編碼**: 利用 **NVENC** 硬體編碼器生成事件影片。針對檔案後處理模式，特別引入了**執行緒化預讀取**與**單次疊加層混合**等優化策略，將影片繪製與編碼的處理速度提升至 **100 FPS** 以上，極大地提高了批次處理效率。
...
2.  **設定行為分析規則 (重要)**:
    打開 `configs/behavior_analysis.yaml` 檔案，根據您的場景需求，設定感興趣區域 (ROI)、虛擬警戒線 (Tripwire) 以及全域錨點策略。檔案內有詳細的註解說明。**此設定對 RTSP 和 FILE 模式同時生效。**
...
-   **[首要] 錨點系統與進階警報 (Anchor System & Advanced Alerts)**:
    -   **可組態錨點系統**: 將錨點策略從目前的全域設定，擴展為可由單條 ROI 或警戒線規則獨立覆寫的精細化設定，以適應更複雜的場景（例如，同時需要判斷腳部位置和身體中心的規則）。
