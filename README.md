...
└── src/                                        # 存放所有專案原始碼
    └── moshousapient/                          # 專案主 Python 套件
        ...
        │   ├── scene_anomaly_processor.py      # [新增] 偵測場景底層異常 (凍結, 遮擋, 篡改)
...
-   **[已完成] 架構演進與核心功能**:
    ...
    -   **[新功能] 多層次智慧警報**: 成功開發了一套包含 AI 行為分析與底層場景異常分析（含篡改檢測）的綜合警報系統。
-   **[進行中] 穩定性與可靠性強化**:
    -   **[已修復] 任務救援競爭條件**: 修正了排程器因超時過短而錯誤重置長耗時任務的 Bug。
-   **[下一步] 架構優化與進階功能**:
    -   **即時分段與通知**: 重構事件處理流程，實現對長事件的即時分段錄製與滾動通知，以增強警報時效性並避免生成單一巨大檔案。
    -   **智慧儲存管理**: 開發一套基於優先級的非同步清理服務，在儲存空間不足時，能智慧地刪除「資訊密度最低」的影片片段。
    -   **白名單系統**: 開發一套白名單機制，以過濾授權人員的正常活動。
-   **技術債**:
