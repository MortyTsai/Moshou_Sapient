...
-   **人物重識別 (Person Re-Identification)**:
    ...
    -   **模型可替換性 (Model Swappability)**: 本系統的 Re-ID 功能在架構上被設計為模型可替換。然而，經過深入調研，目前業界尚不存在可直接「即插即用」的、相容於現有載入器的高性能第三方 Re-ID 模型。所有 SOTA 模型均與其特定的程式碼框架深度綁定。因此，在「架構穩定性優先」的原則下，專案現階段將繼續使用基線模型，並將更高性能模型的整合視為一項獨立的、需進行程式碼級別開發的未來任務。
...
作為一個學習與探索性質的專案，以下是基於當前技術現狀規劃的開發方向：

-   **(優先開發) 行為分析與異常偵測**: 在已實現的 ROI 停留偵測與方向性警戒線基礎上，繼續開發如「人員徘徊」等更複雜的分析模組。此類功能主要依賴穩定的短期追蹤，受 Re-ID 性能影響較小。
-   **(優先開發) 白名單 (Whitelist) 過濾功能**: 建立一個可預先註冊的「熟人」特徵庫。當系統偵測到的人員與白名單匹配時，可選擇抑制警報或以特定標籤記錄，以減少對授權人員的無效告警。此功能的可行性建立在提供多樣化的註冊樣本以對抗 Re-ID 模型的不穩定性。
...
-   **(長期目標 / 依賴 Re-ID 性能) 進階資料庫查詢**: 探索基於 Re-ID 特徵向量的相似度搜尋，以實現特定人物的歷史事件檢索（例如「顯示此人今天所有出現過的片段」）。此功能**高度依賴**一個穩定可靠的 Re-ID 系統。
-   **(長期目標 / 依賴 Re-ID 性能) 多攝影機協同**: 將現有的單攝影機架構擴展為多攝影機管理，並利用特徵庫實現跨攝影機的目標重識別。此功能對 Re-ID 性能的要求**極高**。
-   **(技術債) Re-ID 模型升級**: 根據先前的技術調研，整合一個性能更優越的 SOTA Re-ID 模型（如 OSNet）需要進行程式碼級別的開發。這項任務已被記錄為技術債，待未來專案階段進行。
